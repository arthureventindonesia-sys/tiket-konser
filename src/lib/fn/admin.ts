...
export const fetchConfirmedExport = createServerFn({ method: "POST" })
  .validator((input: { from?: string; to?: string } | undefined) => ({
  ...
  .handler(async ({ data }) => {
    ...
    return listConfirmedExport({ from: data.from || null, to: data.to || null });
  });
